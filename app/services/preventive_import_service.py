        ...
        for row_number in range(8, worksheet.max_row + 1):
            ...
            if _is_untouched_template_row(values):
                ...
                preview.warnings.append(
                    ...
                    'Ganti seluruh isinya dengan data alat sebenarnya.'
                )
                continue
            ...
            if row.asset_name.strip().upper().startswith('CONTOH'):
                ...
                row.match_note = 'Hapus kata CONTOH pada Nama Alat dan isi dengan nama alat sebenarnya.'
...
def _is_untouched_template_row(values):
    """Ignore only the untouched sample row generated by our template."""
    return (
        _norm(values['asset_name']) == _norm('CONTOH - Dental Unit')
        and _norm(values['brand']) == _norm('Contoh Merk')
        and _norm(values['serial_number']) == _norm('CONTOH-SN-001')
        and _norm(values['model']) == _norm('Contoh Type')
        and _norm(values['result']) == _norm('Baik')
        and _norm(values['notes']) == _norm('Hapus baris CONTOH ini, lalu isi data asli.')
    )
