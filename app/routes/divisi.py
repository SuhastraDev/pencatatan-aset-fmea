...
def _build_report_data(room_ids, room_filter, kondisi_filter, rpn_filter):
    ...
    for a in assets:
        ...
        if rpn_filter and context['report_risk_category'] != rpn_filter:
            continue
    ...
    stats = {
        ...
        'rpn_rendah': sum(1 for d in asset_data if d['report_risk_category'] == 'rendah'),
...
def reports_index():
    ...
    for r in rooms:
        ...
        room_summary.append({
            ...
            'rpn_tinggi': sum(1 for d in r_d if d['report_risk_category'] == 'tinggi'),
        })
...
def reports_import():
    ...
    if request.method == 'POST' and request.form.get('action') == 'commit':
        ...
        try:
            ...
            session.pop('asset_report_import_token', None)
            flash(
                f'Import KIB selesai: {result.assets_created} aset baru dibuat, '
                f'{result.assets_updated} aset diperbarui, '
    ...
    if form.validate_on_submit():
        if not form.file.data or not form.file.data.filename:
            ...
            return render_template(
                ...
                template_endpoint='divisi.asset_template',
            )
...
@divisi_bp.route('/reports/import/template')
@login_required
@role_required('admin_divisi')
def asset_template():
    return send_file(
...
def reports_export_excel():
    ...
    for r in rooms:
        ...
        room_stats_sheet.append({
            ...
            'rpn_rendah': sum(1 for d in r_d if d['report_risk_category'] == 'rendah'),
